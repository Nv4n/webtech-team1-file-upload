...
const SERVER_IMAGE_DIR_ROOT_SHARDING = config_1.path.join(config_1.SERVER_IMAGE_DIR_ROOT, 'by_sharding');
function storeImageBySharding(filename) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const clientFilePath = config_1.path.join(config_1.CLIENT_IMAGE_DIR, filename);
            const fileExtension = config_1.path.extname(filename);
            const uuid = (0, config_1.uuidv4)();
            const shard = uuid.substring(0, 2);
            const shardDir = config_1.path.join(SERVER_IMAGE_DIR_ROOT_SHARDING, shard);
            const newFilename = `${uuid}${fileExtension}`;
            const serverFilePath = config_1.path.join(shardDir, newFilename);
            // Create the shard directory if it doesn't exist
            yield config_1.fs.mkdir(shardDir, { recursive: true });
