...
const SERVER_IMAGE_DIR = config_1.path.join(config_1.SERVER_IMAGE_DIR_ROOT, 'by_timestamp');
function storeImageByTimestamp(filename) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const baseFilename = config_1.path.basename(filename, fileExtension);
            const timestamp = Date.now();
            const newFilename = `${baseFilename}_${timestamp}${fileExtension}`;
            const serverFilePath = config_1.path.join(SERVER_IMAGE_DIR, newFilename);
